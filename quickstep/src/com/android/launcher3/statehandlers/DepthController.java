...
public class DepthController implements StateHandler<LauncherState>,
    ...
    private void setDepth(float depth) {
        ...
        if (supportsBlur) {
            final int blur;
            if (mLauncher.isInState(LauncherState.ALL_APPS) && Utilities.getAllAppsScrimAlpha(mLauncher) == 100) {
                ...
                if (Utilities.isBlurOnOverviewEnabled(mLauncher) && (mLauncher.isInState(LauncherState.OVERVIEW) ||
                    mLauncher.isInState(LauncherState.QUICK_SWITCH))) {
                    blur = (int) (mDepth * Utilities.getOverviewScrimBlur(mLauncher));
                } else if (Utilities.isBlurOnAllAppsEnabled(mLauncher) &&
